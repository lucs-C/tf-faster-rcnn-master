...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride, all_anchors, num_anchors):
  ...
  # 以上代码的含义是：将超出原图像尺寸范围的候选窗直接pass掉，保留剩下的窗口，并且创了一个label，用于存放相应窗口的ground truth。
  ...
  # 依据max_overlaps与gt_argmax_overlaps， 根据IOU对当前的窗口进行标定，标注的原则：
  # 1、对每个标定的真值候选区域，与其重叠比例最大的anchor记为前景样本
  # 2、对剩余的anchor，如果其与某个标定重叠比例大于0.7，记为前景样本
  # 3、如果其与任意一个标定的重叠比例都小于0.3，记为背景样本 （处理代码如下：）
  ...
  if len(bg_inds) > num_bg:
    ...
    labels[disable_inds] = -1
  # （到上一行代码截止）经过这样的处理，labels就是包含如下内容：
  #
  ...
  # 这样就可以根据这个labels作为ground truth来使用交叉熵训练RPN网络中的打分支路（就是rpn_cls_prob）

  # （从下面代码开始）接下就要根据样本中的真值窗口搞出来对RPN网络中的窗口回归训练真值：
